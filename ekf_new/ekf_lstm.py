...
from lstm_csv.train_lstm import FlightDataset, load_flight_data  # 从训练脚本中导入 FlightDataset
...
csv_path = os.path.join("D:/code/uav_angle_estimator\\data\\csv_segments", Flight_path)
...
lstm_model.load_state_dict(torch.load("D:/code/uav_angle_estimator\\lstm_csv\\lstm_model.pt", map_location=device))
lstm_model.eval()
...
scaler_x.mean_ = np.load("D:/code/uav_angle_estimator/lstm_csv/input_mean.npy")
scaler_x.scale_ = np.load("D:/code/uav_angle_estimator/lstm_csv/input_std.npy")
scaler_y.mean_ = np.load("D:/code/uav_angle_estimator/lstm_csv/aoa_mean.npy")
scaler_y.scale_ = np.load("D:/code/uav_angle_estimator/lstm_csv/aoa_std.npy")
...
for t in range(N):
    ...
    # ==== LSTM 推理 ====
    with torch.no_grad():
        aoa_pred_norm = lstm_model(input_tensor).cpu().numpy().item()
...
mae = calculate_mae(aoa_true, estimates[:, 1])
...
# plt.plot(aoa_pre, label='LSTM Estimated AOA (rad)', color='red', linestyle='--')
# plt.xlabel("Time Step")
# plt.ylabel("Angle of Attack (rad)")
# plt.legend()
# plt.grid(True)
