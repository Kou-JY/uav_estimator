import numpy as np
from ekf_new.ekf_predict import state_eq, linearization
from ekf_new.ekf_update import ob_eq, linear_mea
...
class EKFCore:
    def __init__(self, GPS_setting='true', object_setting='alpha', x_init=None):
        ...
        if self.GPS_setting == 'false':
            ...
            self.R = np.diag([1e-2, 1e-6])
            # Qx: [vt, alpha, beta] process noise
            # From set_parameter.m: Qx = diag([1E-2 1E-9 1E-9 1E-1]) for object='alpha', GPS='false'
            self.Q = np.diag([1e-3, 1e-6, 1e-5])
        elif self.GPS_setting == 'true':
            self.n_state = 6
            self.R = np.diag([1e-3, 1e-2, 1e-2, 1e-2, 1e-6])
            self.Q = np.diag([1e-3, 1e-6, 1e-5, 1e-12, 1e-10, 1e-12])
        else:
